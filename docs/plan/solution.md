...
**解决的问题**：非标设备报价靠经验，BOM成本和工时成本分开算还是合起来算全看心情，报价低了没利润、报价高了丢单。
...
**解决的问题**：分期付款的账期全靠财务记，销售不知道哪笔款该催了，逾期了也没人管。
...
| **数据迁移** | Excel批量导入+人工核对 | 将现有客户数据（微信记录、Excel表格）迁移至系统，保留历史数据 |
...
- **轻量化**：一期4-6周上线 vs 通用CRM实施周期3-6个月，老板能快速看到效果
...
| 阶段 | 周期 | 核心目标 | 关键交付物 |
|------|------|---------|-----------|
| **启动** | 第1周 | 需求确认与数据准备 | 需求确认书签订、现有客户数据导出、客户现场操作培训 |
| **开发配置** | 第2-4周 | 核心模块上线 | 客户+商机+订单+看板4个模块上线运行 |
| **试运行** | 第5-6周 | 全员使用与问题修复 | 全员试运行、问题收集与修复、老板看板调整 |
| **验收交付** | 第6周末 | 一期验收 | 系统正式上线、操作手册交付、初始数据核对 |
...
| # | 里程碑 | 时间 | 验收标准 |
|---|--------|------|---------|
| 1 | 需求确认完成 | 第1周末 | 双方签字确认需求范围，无重大遗漏 |
| 2 | 核心模块上线 | 第4周末 | 客户、商机、订单、看板4个模块可正常运行 |
| 3 | 全员培训完成 | 第5周末 | 销售和内勤能独立操作系统，老板能查看看板 |
| 4 | 一期验收通过 | 第6周末 | 客户信息入库率≥80%，老板每周查看看板≥3次 |
...
| 2 | 老板期望过高，一次性想解决所有管理问题 | 中 | 中 | 一期明确聚焦范围（客户+订单+看板），管理期望；用分期建设降低每次决策的门槛 |
| 3 | 现有数据质量差（微信记录混乱、Excel表格不规范），迁移成本高 | 中 | 中 | 启动阶段安排1-2天数据清洗，由我方协助整理历史数据，不让客户独自承担迁移负担 |
...
<!-- INTERNAL_NOTES
## 假设前提
- 预算范围基于行业平均水平估算（轻量SaaS方案约5-10万/年，定制开发20万以上），需客户确认
- 客户无现有CRM/ERP系统，从零建设
- 客户接受SaaS部署模式，无需本地服务器
- 销售团队规模2-5人，用户数不多，操作培训周期短
- 老板为最终决策人，决策周期1-4周
- 一期上线时间4-6周为参考周期，具体以签约后项目计划为准

## 知识库引用
- 参考方向：制造业CRM轻量化实施方法论 | 成熟度 L2 | 用于整体思路
- 参考模型：制造业CPQ（Configure-Price-Quote）| 成熟度 L3 | 用于报价管理模块
-->
